...
The mean total number of steps taken each day is `r format(stepsMean, nsmall=2)`  
The median total number of steps taken each day is `r stepsMedian`
...
The 5-minute interval, on average across all the days in the dataset which contains the maximum number of steps is `r intervalTimeMaxAvgSteps`.  
...
The number of rows with NA values is `r countNA`.  
...
With imputation, the mean total number of steps taken each day has increased from `r format(stepsMean, nsmall=2)` to `r format(stepsMeanReplaceNA, nsmall=2)`  

With imputation, the median total number of steps taken each day has increased from `r format(stepsMedian)`
 to `r format(stepsMedianReplaceNA)`
